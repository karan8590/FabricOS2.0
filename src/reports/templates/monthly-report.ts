import { MonthlyReportData } from '../utils/analytics';
import { formatCurrency, formatPercentage } from '../utils/formatting';

export function renderMonthlyReport(data: MonthlyReportData): string {
    const orderRows = data.orders.length === 0
        ? `<tr><td colSpan="7" class="px-4 py-8 text-center text-slate-400 font-medium bg-white">No orders found for this monthly period.</td></tr>`
        : data.orders.map(o => {
...
<html lang="en">
    <head>
        <meta charset="UTF-8" />
        <title>FabricOS Monthly Summary</title>
    ...
    <body class="text-slate-800 p-6">
        ...
        <div class="border-b border-slate-100 pb-6 mb-8 flex justify-between items-end">
            <div>
                ...
                <h1 class="text-3xl font-extrabold text-navy-900 tracking-tight">Monthly Operations &amp; Financial Summary</h1>
                <p class="text-sm text-slate-500 mt-1 font-medium">Report period: ${data.monthName}</p>
            </div>
            <div class="text-right">
                <span class="text-xs uppercase tracking-wider text-slate-400 font-bold">Confidential Enterprise Report</span>
        ...
        <div class="grid grid-cols-4 gap-4 mb-8">
            ...
            <div class="bg-slate-50 rounded-xl p-4 border border-slate-100 shadow-sm relative overflow-hidden">
                ...
                <div class="text-xs font-medium text-slate-500 mt-1">Salaries: ${formatCurrency(data.kpis.salaryPaid)}</div>
            </div>

            <div class="bg-slate-50 rounded-xl p-4 border border-slate-100 shadow-sm relative overflow-hidden">
                <div class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-amber-500 to-orange-500"></div>
                <span class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Outstanding Amount</span>
                <div class="text-2xl font-bold text-navy-900 mt-1">${formatCurrency(data.kpis.outstanding)}</div>
                <div class="text-xs font-medium text-slate-500 mt-1">Awaiting Collection</div>
            </div>
            ...
            <div class="bg-slate-50 rounded-xl p-4 border border-slate-100 shadow-sm relative overflow-hidden">
                ...
                <div class="text-xs font-medium text-slate-500 mt-1">Salary + General Expenses</div>
            </div>
        </div>

        <!-- Monthly Summary Banner and Stats -->
        <div class="bg-slate-50 rounded-xl p-5 border border-slate-100 shadow-sm flex flex-row items-center justify-between gap-6 mb-8">
            <div>
                <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Monthly Order Volume Summary</h3>
                <div class="flex items-baseline gap-2">
                    <span class="text-4xl font-extrabold text-navy-900">${data.totalOrders}</span>
                    <span class="text-sm font-semibold text-slate-500">Total Orders Placed</span>
                </div>
            </div>
            <div class="text-xs font-semibold text-slate-500 bg-white border border-slate-200/60 rounded-lg p-3">
                Top Customer: <span class="text-navy-900 font-bold">${data.topCustomerName}</span>
            </div>
        ...
        <div>
            <div class="flex justify-between items-center mb-3">
                <h3 class="text-sm font-extrabold text-navy-900 uppercase tracking-wider">All Monthly Orders Details</h3>
