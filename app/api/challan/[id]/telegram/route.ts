...
export async function POST(
    ...
    try {
        ...
        const telegramText = `📦 *Dispatch Created*\n\n` +
            ...
            `*Driver:* ${challan.driver_name || 'N/A'}`;
        ...
        if (telegramSent) {
            await db.prepare('UPDATE dispatch_challans SET telegram_sent = 1 WHERE id = ?').run(challanId);
        ...
        console.error('Challan Telegram API Error:', error);
