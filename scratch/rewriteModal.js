...
export default function InventoryHistoryModal({ isOpen, onClose, materialId, materialName, unit }: InventoryHistoryModalProps) {
    ...
    const totalPurchased = history.filter(r => r.action_type === 'Purchase').reduce((sum, r) => sum + Number(r.quantity || 0), 0);
    const totalReserved = history.filter(r => r.action_type === 'Reserved').reduce((sum, r) => sum + Number(r.quantity || 0), 0);
    const totalUsed = history.filter(r => ['Used', 'Consumed', 'Consumption'].includes(r.action_type)).reduce((sum, r) => sum + Number(r.quantity || 0), 0);
    const totalProcurementValue = history.filter(r => r.action_type === 'Purchase').reduce((sum, r) => sum + Number(r.total_cost || 0), 0);
    const currentStock = history.length > 0 ? history[0].new_stock : 0;
...
`;
fs.writeFileSync('/Users/karandhameliya/Desktop/ag/FabricOS/components/inventory/InventoryHistoryModal.tsx', content);
